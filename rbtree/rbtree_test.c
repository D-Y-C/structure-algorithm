...
void dfs_show_node(struct rb_node  *node,int spaces)
{
	int loop ;
	...
	dfs_show_node(node->rb_right,spaces + 4); 
				    
	for(loop = 1; loop <= spaces; loop++){
		printf("  ");
	}   
	struct my_node* test = container_of(node,struct my_node,rb_node);
	...
	dfs_show_node(node->rb_left,spaces+4);
								    
	node = NULL;
}

void show_tree(struct rb_root *root)
{
	dfs_show_node(root->rb_node,0);
	printf("\n");
}

void main()
{
    int a[] = {10, 40, 30, 60, 90, 70, 20, 50, 80,40,0,110,35,45};
    int i, ilen=LENGTH(a);
